...
The application allows you to monitor your system processes.
It shows such parameters as:
    1) Command name, which created process
    2) User, who has started selected process
    3) Process ID (PID)
    4) Parent process ID (PPID)
    5) Process priority
    6) Process niceness
    7) Process status
    8) Absolute value of RAM consumed by process
    9) Relative value of RAM consumed by process
    10) Process CPU consuming
    11) Processor time occupied by selected process
Navigation:
    Application supports two kinds of navigation: arrow-keys controls and
    VIM-based h,j,k,l controls. To move home/end one can use Home/End buttons
    or K/J hotkeys. To exit the application use 'q' or ESC.
Update:
    By default the app updates process list with every symbol you type. If you
    want to disable such behaviour use 'p' button. It prevents process list
    from updating unless you type 'p' one more type. Use 'm' button to make
    "force update" for process list.
Sort:
    The app supports sorting by pressing 's' button. It offers you to enter
    parameter name and sorts processes on next update of process list. To get
    inversed sorting just type 'b' button.
Control:
    The allows you to control processes by sending signals. To send signal you
    need to press 'x' and enter signal number. Supported signals described in
    right window.
Search:
    To find needed process type '/' and enter process name.
"""
